...
test('player notes view resize handles adjust both sidebars', async ({ browser }) => {
	const dm = await browser.newContext();
	const page = await dm.newPage();
	await page.goto('/login');
	await page.fill('input[type=password]', 'test-passcode');
	...
	const outline = page.locator('.frame > aside.rail:not(.rolls)');
	const rolls = page.locator('.frame > .rail.rolls');
	await expect(outline).toBeVisible();
	await expect(rolls).toBeVisible();
	await page.waitForTimeout(200);
	...
	console.log('PLAYER OUTLINE', outlineBefore, '->', outlineAfter);
	console.log('PLAYER ROLLS', rollsBefore, '->', rollsAfter);
	expect(outlineAfter).toBeGreaterThan(outlineBefore + 40);
	expect(rollsAfter).toBeGreaterThan(rollsBefore + 40);
	await dm.close();
});
